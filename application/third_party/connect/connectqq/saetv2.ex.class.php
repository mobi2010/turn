 ...
 * @ignore
 */
class ConnectqqOAuthException extends Exception {
	// pass
}
...
class ConnectqqSaeTOAuthV2 {
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		$params['scope'] = 'get_user_info,add_share,list_album,add_album,upload_pic,add_topic,add_one_blog,add_weibo,check_page_fans,add_t,add_pic_t,del_t,get_repost_list,get_info,get_other_info,get_fanslist,get_idolist,add_idol,del_idol,get_tenpay_addr';
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new ConnectqqOAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			$openid = $this->get('https://graph.qq.com/oauth2.0/me',array('access_token'=>$token['access_token'],'decode_json'=>false));
			if($openid){
				preg_match('/"openid":"(.*?)"/is', $openid, $matches);
				$token['openid'] = $matches[1];//用户身份的标识
		    }			
			//$this->refresh_token = $token['refresh_token'];
		} else {
			throw new ConnectqqOAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
	...
	function get($url, $parameters = array()) {
		...
		if($parameters['decode_json'] === false){
			return $response;
		}
...
class ConnectqqSaeTClientV2
{
	 ...
	 * @return void
	 */
	function __construct( $akey, $skey, $access_token, $refresh_token = NULL,$openid=null)
	{
		$this->oauth = new ConnectqqSaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
		$this->params = array(
	 ...
	 * 获取登录用户的听众列表
	 */
	function get_fanslist($params=array()){
		$params = $params + $this->params;
		return $this->oauth->get('https://graph.qq.com/relation/get_fanslist',$params);
	}
	/**
	 * 获取登录用户收听的人的列表
	 */
	function get_idollist($params=array()){
		$params = $params + $this->params;
		return $this->oauth->get('https://graph.qq.com/relation/get_idollist',$params);
	}
	/**
	 * 获取用户在QQ空间的个人资料
	 */
	function get_user_info($params=array()){
		$params = $params + $this->params;
		return $this->oauth->get('https://graph.qq.com/user/get_user_info',$params);
	}
	//同步分享到QQ空间、腾讯微博
	function add_share($params=array()){
		$params = $params + $this->params;
		return $this->oauth->post('https://graph.qq.com/share/add_share',$params);
	}
	// =========================================
