...
class DoubanSaeTOAuthV2 {
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		$params['scope'] = 'douban_basic_common,community_basic_note,community_basic_user,community_basic_photo,community_basic_online,shuo_basic_r,shuo_basic_w';
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new DoubanOAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new DoubanOAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
...
class DoubanSaeTClientV2
{
	...
	{
		$this->oauth = new DoubanSaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
		$this->akey = $akey;
	}
	...
	function get_user_info($uid){		
		return $this->oauth->get('v2/user/'.$uid);
	}
	// 发送一条广播
	function add_shuo($params){
		$params['source'] = $this->akey;
		return $this->oauth->post('shuo/v2/statuses/',$params,true);
	}
	//获取广播列表
	function get_shou_list($uid=null){
		return $this->oauth->get('shuo/v2/statuses/user_timeline/'.$uid);
	}
	// =========================================
