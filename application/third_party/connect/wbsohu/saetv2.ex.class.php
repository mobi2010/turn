 ...
 * @ignore
 */
class WbsohuOAuthException extends Exception {
	// pass
}
...
class WbsohuSaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	function accessTokenURL()  { return 'https://api.t.sohu.com/oauth2/access_token'; }
	/**
	 * @ignore
	 */
	function authorizeURL()    { return 'https://api.t.sohu.com/oauth2/authorize'; }
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new WbsohuOAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new WbsohuOAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
...
class WbsohuSaeTClientV2
{
	...
	{
		$this->oauth = new WbsohuSaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
	}
	...
	{	
		return $this->oauth->get('statuses/friends', $params);//可能是接口的bug不能补全
	}
	/**
	 ...
	 * @return array
	 */
	function show_user_by_id( $uid=array() )
	{
		$params=array();
		if ( $uid !== NULL ) {
			$this->id_format($uid);
