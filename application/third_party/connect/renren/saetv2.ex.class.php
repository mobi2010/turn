 ...
 * @ignore
 */
class RenrenOAuthException extends Exception {
	// pass
}
...
class RenrenSaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	public $host = "https://api.renren.com";
	/**
	 * Set timeout default.
	 *
	 * @ignore
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		$params['scope'] = "read_user_message send_notification write_guestbook send_request read_user_blog read_user_album read_user_checkin send_message read_user_status read_user_notification read_user_share send_invitation photo_upload publish_comment publish_blog read_user_comment create_album read_user_guestbook publish_share read_user_like_history operate_like read_user_feed read_user_photo read_user_invitation status_update publish_checkin read_user_request publish_feed";
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new RenrenOAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new RenrenOAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
...
class RenrenSaeTClientV2
{
	...
	{
		$this->oauth = new RenrenSaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
	}
	/**
	 * [状态]
	 * @return [type] [description]
	 */
	function feed_list($params=array()){
		return $this->oauth->get('/v2/feed/list', $params);//可能是接口的bug不能补全
	}
	/**
	 * [发布状态]
	 * @return [type] [description]
	 */
	function feed_put($params=array()){
		return $this->oauth->post('/v2/feed/put', $params);//可能是接口的bug不能补全
	}
	/**
	...
	function get_user_info($params=array()){
		return $this->oauth->get('/v2/user/get', $params);
	}
	/**
	 * [friend_list 获取某个用户的好友列表]
	 * @return [type] [description]
	 */
	function friend_list($params=array()){
		return $this->oauth->get('/v2/user/friend/list', $params);
	} 
	/**
	 * 开启调试信息
	 *
