...
<?php
echo html_tags(array('checked'=>$city_id,'name'=>'cid','sval'=>'name','class'=>'btn-grey-s','options'=>$dataCity,'blank'=>'&nbsp;','href'=>mobi_query_url('admin/services/index',array('cid'))));
    ...
    <?php
        if(!empty($dataList)){
            foreach ($dataList as $key => $value) {
                ...
                $view = html_a(array('href'=>mobi_url('services/detail',array('id'=>$city_id.'_'.$value['id'])),'text'=>'view','target'=>'_blank'));
                echo <<<ETO
                <tr>
    ...
    $("input[name='ckbOption[]']").click(function(){
     ...
     $('#deleteBtn,#updateBtn').click(function(){
            ...
            if(id == 'deleteBtn' && confirm("确定删除?")){
                $.post("<?=base_url('admin/services/delete');?>",$('#servicesForm').serialize(),function(dt){
                    $.mobi.alert(dt);
                    $.mobi.refresh();
                })                
                return false;
            ...
            if(id == 'updateBtn'){
                $.post("<?=base_url('admin/services/update');?>",$('#servicesForm').serialize(),function(dt){
                    $.mobi.alert(dt);
                    $.mobi.refresh();
                })                
                return false;
