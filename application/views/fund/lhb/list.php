<?php
$fundFields['total'] = ['title'=>'上榜次数'];

$fundFields += $this->initData['dataLhb']['fields'];

$body .= "日期:".html_text(['name'=>'startDate','value'=>$startDate,'onClick'=>"WdatePicker({dateFmt:'yyyy-MM-dd'})"]);

$body .= "到".html_text(['name'=>'endDate','value'=>$endDate,'onClick'=>"WdatePicker({dateFmt:'yyyy-MM-dd'})"]);

$body .= html_submit(['value'=>'筛选']);


$body .= str_repeat("&nbsp;",10).html_a(['text'=>'返回','href'=>mobi_url('lhb/index')]);

echo html_form(['body'=>$body,'method'=>'get','action'=>mobi_url('lhb/analysis')]);
