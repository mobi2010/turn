...
<div class="home-body">
    <div class="home-body-box">
        ...
        $retrieval .= html_div(array('style'=>'margin-bottom:5px','body'=>'类型：'.html_tags(array('checked'=>$typeChecked,'name'=>'tid','sval'=>'name','class'=>'btn-grey-s','options'=>array(0=>array('name'=>'全部'))+$dataCar["type"],'blank'=>'&nbsp;','href'=>mobi_query_url('car/lists',array('tid'))))));
        if($modeChecked%2 == 1){
            $retrieval .= html_div(array('style'=>'margin-bottom:5px','body'=>'卧室：'.html_tags(array('checked'=>$roomChecked,'name'=>'rid','class'=>'btn-grey-s','options'=>$roomData,'blank'=>'&nbsp;','href'=>mobi_query_url('car/lists',array('rid'))))));
        }
        echo html_div(array('body'=>$retrieval,'style'=>'padding:20px'));

        if(empty($dataList)){
            echo html_div(array('body'=>"没有数据,去".html_a(array('text'=>'发布信息','href'=>base_url('member/publish/car')))));
        }else{   
            $upImg = html_img(array('src'=>'/style/img/up.ico','height'=>'12px'));
            $downImg = html_img(array('src'=>'/style/img/down.ico','height'=>'12px'));
            $sort = '排序：时间['.html_a(array('href'=>mobi_query_url('car/lists',array('ob','ud'),array('ob'=>1,'ud'=>1)),'text'=>$upImg)).'|'.html_a(array('href'=>mobi_query_url('car/lists',array('ob','ud'),array('ob'=>1,'ud'=>2)),'text'=>$downImg)).']';
            $mark = '售价';
            $ob = 2;
            $sort .= '&nbsp;&nbsp;'.$mark.'['.html_a(array('href'=>mobi_query_url('car/lists',array('ob','ud'),array('ob'=>$ob,'ud'=>1)),'text'=>$upImg)).'|'.html_a(array('href'=>mobi_query_url('car/lists',array('ob','ud'),array('ob'=>$ob,'ud'=>2)),'text'=>$downImg)).']';
            ...
            foreach ($dataList as $key => $value) {
                ...
                $image = html_div(array('body'=>html_a(array('text'=>html_img(array('src'=>$imgSrc,'width'=>'160px','height'=>'120px')),'href'=>base_url('car/detail/?id='.$car_id.$value['id']),'target'=>"_blank"))));   
                
                $field_1 = html_dd(array('body'=>$image,'class'=>'field1'));


                $title = html_div(array('body'=>html_a(array('text'=>$value['title'],'href'=>base_url('car/detail/?id='.$car_id.$value['id']),'target'=>"_blank")),'class'=>'title'));
...
<script type="text/javascript">
    $(document).ready(function() {  
        $('#search').click(function(){
            var q = $('#q').val();           
            $.mobi.location("<?=mobi_query_url('car/lists',array('q'))?>&q="+q);
            return false;
        })        
    })
</script>