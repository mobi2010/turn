...
<div class="member-body">
    ...
    <div class="member-content">
        ...
        $colspan = 6;
        if(empty($listData)){
            $th = html_th(array('colspan'=>$colspan,'body'=>"没有数据,去".html_a(array('text'=>'发布信息','href'=>base_url('member/publish/services')))));
            ...
            foreach ($listData as $key => $value) {
                    ...
                    $td .= html_td(array('body'=>html_a(array('text'=>$value['title'],'href'=>base_url('services/detail/?id='.$services_id.$value['id']),'target'=>"_blank")),'align'=>'center'));
                    $td .= html_td(array('body'=>$dataServices['type'][$value['type']]['name'],'align'=>'center'));
    ...
    $(document).ready(function() {  
        ...
        $('#flash,#delete').click(function(){
            ...
            if(id == 'delete' && confirm("确定删除?")){
                $.post("<?=base_url('member/index/deleteServices');?>",$('#listDataForm').serialize(),function(dt){
                    $.mobi.alert(dt);
                    $.mobi.refresh();
                })                
                return false;
            ...
            if(id == 'flash'){
                $.post("<?=base_url('member/index/flashServices');?>",$('#listDataForm').serialize(),function(dt){
                    $.mobi.alert(dt);
                    $.mobi.refresh();
                }) 
                return false;
